...
```{r, eval=FALSE}
install.packages(c(
  "broom.mixed", "dplyr", "ggplot2", "lme4", "nngeo",
  "purrr", "rlang", "scales", "sf", "sp", "stringr",
  "tibble", "tidygeocoder", "tidyr"
))
```
...
::: {.callout-note title="Rendering modes: `real_data` parameter"}
...
::: {.callout-note title="Simulated data mode"}
The code shown in Parts 1–3 reflects the full workflow as it would run with the original OSM and POI-Bund data (`real_data: true`). In this rendering (`real_data: false`), the code is displayed for illustration but executed silently using pre-saved outputs. All printed diagnostics, bar charts, and maps shown below are based on the real data and were saved during a prior run with `real_data: true`.
:::
...
```{r, echo=FALSE}
# Helper functions and constants — only needed for the real data path
...
The code above shows how to merge POI counts from Parts 1–3 onto individual survey respondents. If you are working with your own georeferenced survey data, run this code after completing Parts 1–3. The only requirement is a shared municipality identifier (`mun_id`) across the survey and POI data. After the join, respondents in municipalities where a given POI type was not recorded will have `NA` on the corresponding count variable, which is addressed below via zero imputation.

In this tool, Part 4 always runs on the three simulated datasets (`sim_survey`, `sim_osm_poi`, `sim_official_poi`) joined by `mun_id`, as shown in the code block below.
:::

For this tool, Part 4 always runs on simulated data. The simulated survey responses are linked to the simulated POI datasets via the shared municipality identifier:

```{r}
sim_survey   <- readRDS("./data/sim_survey.rds")
sim_osm      <- readRDS("./data/sim_osm_poi.rds")
sim_official <- readRDS("./data/sim_official_poi.rds")
...
controls_table <- tibble::tribble(
  ...
  "Education: low",      pct(survey_context$edu_low),                                   "—",
  "Education: mid",      pct(survey_context$edu_mid),                                   "—",
  "Education: high",     pct(survey_context$edu_high),                                  "—"
)
