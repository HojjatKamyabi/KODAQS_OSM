...
::: {.callout-tip}
## Key Concept: Public Service Provision

Public service provision refers to the **local availability of essential services**, operationalized as the presence of:

- Hospitals
- Schools
- Supermarkets

within the respondents' **immediate residential environment**.
:::
...
```{mermaid}
flowchart LR
    A[GLES Data] --> B[Assign Municipality]
    B --> C[Link OSM and Official POI Data by Municipality]
    C --> D[Create Public Service Provision Proxies]
    D --> E[Run Regression Analyses]
    E --> F[Compare Results for OSM and Official POI Proxies]
```
...
for (nm in names(files)) { 
  assign(nm, st_read(dsn = file.path("./data", files[[nm]]),
                     ...
                     # features drawn as line/point will be written to 
                     # the layer without an _a suffix
...
for (nm in names(files)) {  
  assign(paste0(nm, "_poly"),
         st_read(dsn = file.path("./data", files[[nm]]),
                 ...
                 # features drawn as area will be written to 
                 # the layer with an _a suffix
...
::: {.callout-important title="Major Take-Away (2)"}
...
::: {.callout-note title = "Major Implications"}
...
**2. Robustness across Data Sources**
...
Three main findings are summarized here:

**First**, OSM-based POI data and official built environment data provide **largely comparable coverage** of public service provision proxies at both the **national and municipal levels**.

**Second**, the small differences in coverage primarily reflect **differences in data production and tagging practices**. OSM contributors and official data providers apply distinct rules when recording, classifying, and tagging built environment features, including how facilities such as schools, hospitals, or supermarkets are defined conceptually and geographically. Despite this conceptual non-equivalence, our results suggest that the datasets can be treated as interchangeable for this application.

**Third**, regression estimates derived from **OSM-based and POI-Bund proxies** yield **highly similar results across all public service provision indicators**.
